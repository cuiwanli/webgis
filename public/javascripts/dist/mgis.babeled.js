...
require(["esri/Map", "esri/views/MapView", "esri/views/SceneView", "esri/layers/GraphicsLayer", "esri/geometry/Polyline", "esri/symbols/SimpleLineSymbol", "esri/Graphic", "esri/widgets/Popup", "esri/PopupTemplate", "dojo/domReady!"], function (Map, MapView, SceneView, GraphicsLayer, Polyline, SimpleLineSymbol, Graphic, Popup, PopupTemplate) {
    function MarianaGis() {
        var myGis = {
            baseType: 'oceans',
            viewType: '3D',
        ...
        function addNewGraphic(layerId, data) {
            ...
            var newContent = '<div>start: lat=' + data.path[0][0] + ' lon=' + data.path[0][1] + ' alt=' + data.path[0][2] + '</div>' + '<div>end: lat=' + data.path[1][0] + ' lon=' + data.path[1][1] + ' alt=' + data.path[1][2] + '</div>' + '<img class="prev-img" src=' + data.img.substring(0, data.img.lastIndexOf('.')) + "-min.jpg" + ' alt="image preview" data-raw=' + data.img + ' onclick="showImg(this)" />' + '<a href=' + data.img + ' download=' + data.img.substring(data.img.lastIndexOf('/')) + '><span class="glyphicon glyphicon-save" />download</a>';
        ...
        function highLightGraphic(graphic) {
            if (!graphic.originId) {
                //only highlighted graphics have attr'originId'
                this.unHighLightAll();
                var lineSymbol = new SimpleLineSymbol({
                    color: [255, 0, 0],
                    width: 5
